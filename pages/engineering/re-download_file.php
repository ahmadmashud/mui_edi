				...
				{		
					...
					{	
						...
						$th ="SELECT * FROM tb_transaction_history where document_id='$data[document_id]'";
								...
								$uploader = $rows_th['uploader'];
						  
								$email  = "SELECT email FROM tb_user WHERE supplier='Project Control' and section='Project Control' and full_name='$uploader'";
								...
								$email_from="requestre-download-notification-noreply@ptmui.co.id";
								$email_to = $row['email'];
								$email_subject = "Request Re-download Notification";
								$email_message = "
								Dear ".$uploader.",
								
								This notification just to remind that there are some Document that the supplier wants to download again with the 
								Document Number: ".$rows_th['document_number']."
								and Document Name: ".$rows_th['document_name']."
								with Reference Number: ".$rows_th['reference_number']."
