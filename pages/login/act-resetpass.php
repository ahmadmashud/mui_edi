...
if(isset($_POST['submit']))
{
	...
	{	
		...
		if(mysql_num_rows($sql)==1)
		{
			...
			echo"<script>alert('Thank you for requesting reset password submission. Your request will be processed immediately and we will send you a new password information to your email as soon as possible.')</script>";
			echo"<script>onclick=location.href='../../index.php'</script>";
		}
			else
		{	
			echo"<script>alert('Sorry, email never registered.')</script>";
			echo"<script>onclick=location.href='../../resetpass.php'</script>";
		}
	}
		else
	{
