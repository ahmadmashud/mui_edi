				...
				<div class="row">
					...
					<div class="box">              	
						...
						<div class="box-body">					
							...
							<?php						
							error_reporting(0);
							$select_sds_details = mysqli_query($conn,"SELECT * FROM tb_supplier_delivery_schedule WHERE shipment_status!='COMPLETELY RECEIVED'");
							...
							{ 
							$select_sds_fifo = mysqli_query($conn,"SELECT * FROM tb_supplier_delivery_schedule WHERE sds_status='OPEN' order by sds_number");
							...
							$select_sds_details1 = mysqli_query($conn,"SELECT * FROM tb_supplier_delivery_schedule WHERE shipment_status!='COMPLETELY RECEIVED' and shipment_status='ON THE WAY'");
							$sds_details_count1 = mysqli_num_rows($select_sds_details1);
							...
							?>						  
							<?php
								$show1 = mysqli_query($conn,"SELECT * FROM tb_supplier_delivery_schedule_details order by sds_number");	
							...
							?>					
								...
								<div style="overflow-x:auto;"> 
									...
									$tampilPeg=mysqli_query($conn,"SELECT * FROM tb_supplier_delivery_schedule_details where po_number = '$po_number' and sds_number='$sds_number' ORDER BY sds_number");
									$no = 1;
									...
									?>		
											<tr class="body">
												...
												<td width=14%>
													<input type="text" onkeypress="return isNumberKey(event)" name="quantity[]" size="12px" value="<?php echo $peg['outstanding_delivery'];?>" required>
												</td>
							...
							<?php		  			  
								...
								$sql =  "SELECT * FROM tb_supplier_delivery_order WHERE do_number LIKE '%$keyword%' ORDER BY do_date DESC";
								$result = mysqli_query($conn,$sql);
								}else{
								$reload = "do_product.php?pagination=true";
								$sql =  "SELECT * FROM tb_supplier_delivery_order ORDER BY do_date DESC";
								$result = mysqli_query($conn,$sql);
								}
							...
							<div>
								...
								<div id="popup">
									<div class="window">
										...
										<?php
										$min_tanggal=mysqli_fetch_array(mysqli_query($conn,"select min(do_date) as min_tanggal from tb_supplier_delivery_order"));
										$max_tanggal=mysqli_fetch_array(mysqli_query($conn,"select max(do_date) as max_tanggal from tb_supplier_delivery_order"));
										?>
										...
										?>
												...
												<?php
												$query=mysqli_query($conn,"select * from tb_supplier_delivery_order where do_date between '$tanggal_awal' and '$tanggal_akhir' order by do_date");
											}
											?>			
										...
										<div style='height:48%;overflow:auto;'>	
											...
											?>
												<tr class="body">
													...
													<td width=3%>
														<a href="<?php echo 'export_csv_do.php?id='.$row['do_number_id'];''?>"><center><i class="fa fa-download"></i></center></a>
													</td>
												</tr>
											<?php
											}
