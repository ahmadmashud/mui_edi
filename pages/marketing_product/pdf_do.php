...
$net = new mysqli($host, $user, $pass, $dbnm,$port);

 if($net->connect_error){
  die("Connection failed: ".$net->connect_error);
 }
	
	if (isset($_GET['do_number'])) {
	...
	$q = "select * from tb_supplier_delivery_order where do_number='$do_number'";
   ...
   $this->Cell(19,1,'SUPPLIER DELIVERY ORDER',0,0,'C');
   $this->Ln(1.3);
   $this->SetFont('Arial','B','10');
   $this->Cell(19,1,'DO Number : ' .$do_number,0,0,'L');
   $this->Ln(0.7);
   $this->Cell(15,1,'DO Date : ' ."N/A",0,0,'L');
   $this->Ln(1.2);
   $this->SetFont('Arial','B','10'); 
